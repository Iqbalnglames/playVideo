...
@section('content')
    ...
    <div class="bg-white p-6 rounded-2xl shadow-md">
        ...
        @endif
        <form action="{{ route('video.store') }}" enctype="multipart/form-data" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm mb-1">Judul</label>
                <input type="text" name="title" class="w-full border p-2 rounded-lg">
            </div>

            <div>
                <label class="block text-sm mb-1">Thumbnail</label>
                <input type="file" accept="image/*" name="thumbnail" class="w-full border p-2 rounded-lg">
            </div>

            <div>
                <label class="block text-sm mb-1">File Video</label>
                <input type="file" accept="video/*" name="video" class="w-full border p-2 rounded-lg">
            </div>
